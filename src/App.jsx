...
const DISTRICTS = {
  rda: {
    ...
    schoolTax: "Captured 100% — state backfilled school shortfall (what made it expensive)",
    ...
    examples: [
      ...
      { name: "SF Redevelopment Agency", year: 1948, reason: "Post-WWII blight, Western Addition", notes: "Notoriously displaced thousands of Black residents in the 1960s–70s" },
  ...
  ifd: {
    ...
    eligibleSpend: ["Capital facilities with useful life ≥ 10 years","Public infrastructure (roads, bridges, parks)","Industrial and commercial buildings","Removal of hazardous waste","Facilities with regional benefit"],
    death: "IFDs were almost never used because the 2/3 voter approval requirement made formation practically impossible. When RDA was abolished in 2012, the state urgently needed a replacement and reformed IFD into EIFD — stripping the voter threshold and adding proper governance.",
  ...
  eifd: {
    ...
    moneyRaised: "Property tax increment (non-school entities only); revenue bonds secured by increment only — NOT by city/county general fund; no new taxes",
    eligibleSpend: ["Public capital facilities with useful life ≥ 15 years","Transportation infrastructure","Affordable housing","Sewage and water treatment","Environmental mitigation","Climate adaptation projects","Economic development","Parks and recreation"],
    examples: [
...
const CRD_DRD_ROWS = [
  ...
  },
  {
    ...
    crd: "Any time — no deadline",
    drd: "Resolution must be adopted within 2 years of disaster proclamation",
  },
  {
    feature: "Formation Timeline",
    crd: "1–2 years (standard EIFD process)",
    drd: "Expedited — months, not years",
  },
  {
    feature: "Protest Rights",
    crd: "Preserved: < 25% approved | 25–50% → election | > 50% → halted",
  ...
  },
  {
    ...
    crd: "Any area facing climate risk — no special constraint",
    drd: "Must be within declared disaster area. Adjacent areas ≤ 20% of total",
  },
  {
    feature: "Eligible Spending — Climate",
    crd: "✓ Sea level rise, heat, wildfire, drought, flood, cold",
    drd: "✓ All CRD climate projects (inherited)",
  },
  {
    feature: "Eligible Spending — Rebuilding",
    crd: "✗ Cannot fund repair/replacement of damaged structures",
    drd: "✓ Acquiring, demolishing, repairing disaster-damaged structures",
  },
  {
    feature: "Eligible Spending — Housing",
    crd: "✗ Not explicitly included",
    drd: "✓ Low- and moderate-income housing damaged by disaster",
  },
  {
    feature: "Eligible Spending — Utilities",
    crd: "✗ Not explicitly included",
    drd: "✓ Undergrounding and hardening electrical lines and utilities",
  },
  {
  ...
  },
  {
    ...
    crd: "✓ With voter approval",
    drd: "✓ Inherited from CRD (voter approval still required)",
  },
  {
    feature: "Annual Budget Plan Required",
    crd: "✓ Yes — annual expenditure plan, operating + capital budgets",
    drd: "✓ Inherited from CRD",
  },
  {
  ...
  return (
    <div style={{ overflowX: "auto" }}>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", maxWidth: W, display: "block" }}>
        ...
        <text x="185" y={nc("rda").y - 18} fill={C.rda} fontSize="9" fontFamily="Georgia, serif" opacity="0.5" textAnchor="middle">replaced by →</text>
        ...
        <text x="190" y={nc("ifd").y + 36} fill={C.ifd} fontSize="9" fontFamily="Georgia, serif" opacity="0.5" textAnchor="middle">legal DNA →</text>
        ...
        <div style={{ padding: "12px 16px", borderTop: `1px solid ${C.border}` }}>
          ...
          {Object.values(DISTRICTS).map(dist => (
            <div key={dist.id} onClick={() => scrollToDistrict(dist.id)}
              ...
              <span style={{ fontSize: 10, color: T.faint }}>{dist.year} · {dist.label}</span>
            </div>
          ))}
        </div>
      </aside>
        ...
        <div style={{ padding: "44px 44px 100px", maxWidth: 860 }}>
          ...
          <div style={{ marginBottom: 56 }}>
            ...
            <p style={{ fontSize: 14, color: T.sub, lineHeight: 1.8, maxWidth: 560, margin: "0 0 20px" }}>
              From the original Redevelopment Agency (1945) to the Disaster Recovery District (2025) — each generation built on the last, fixing what broke, adding what was missing.
            </p>
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              {Object.values(DISTRICTS).map(dist => (
                <div key={dist.id} onClick={() => scrollToDistrict(dist.id)} style={{
                  ...
                  <div style={{ fontSize: 10, color: T.faint, marginTop: 1 }}>{dist.year}{dist.endYear ? `–${dist.endYear}` : "–"}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <section ref={setRef("timeline")} data-section="timeline" style={{ marginBottom: 70 }}>
            ...
            <p style={{ fontSize: 13, color: T.sub, marginBottom: 22, lineHeight: 1.7 }}>
              RDA and IFD ran <em>in parallel</em> — not parent-child. EIFD absorbed both when RDA was killed. CRD and DRD are nested subtypes inside EIFD. Click any node to jump to its deep-dive.
            </p>
          ...
          <section ref={setRef("tree")} data-section="tree" style={{ marginBottom: 70 }}>
            ...
            <div style={{ marginTop: 14, background: "#fffbeb", borderRadius: 8, padding: "13px 17px", border: "1px solid #fde68a" }}>
              <span style={{ fontSize: 12, color: "#92400e" }}>
                <strong>Key insight:</strong> RDA and IFD were siblings (1990–2012), not parent-child. When RDA died in 2012, EIFD was purpose-built to replace its function — borrowing IFD's legal framework but removing the voter threshold and excluding the school tax backfill that made RDA unsustainable.
              </span>
            </div>
          </section>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
              {Object.values(DISTRICTS).map(dist => (
                <button key={dist.id}
                  ...
                  {compareFilter.includes(dist.id) ? "✓ " : ""}{dist.label}
                </button>
              ))}
          ...
          <section ref={setRef("crd-drd")} data-section="crd-drd" style={{ marginBottom: 70 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 4 }}>
              <div style={{ background: `${C.crd}0e`, border: `2px solid ${C.crd}30`, borderRadius: 10, padding: "16px 20px" }}>
                ...
                <div style={{ fontSize: 12, color: T.sub }}>Climate Resilience District · <a href="https://leginfo.legislature.ca.gov/faces/billNavClient.xhtml?bill_id=202120220SB852" target="_blank" rel="noopener noreferrer" style={{color:C.crd,fontWeight:600,textDecoration:"underline",textUnderlineOffset:2}}>SB 852 (2022) ↗</a></div>
              ...
              <div style={{ background: `${C.drd}0e`, border: `2px solid ${C.drd}30`, borderRadius: 10, padding: "16px 20px" }}>
                ...
                <div style={{ fontSize: 12, color: T.sub }}>Disaster Recovery District · <a href="https://leginfo.legislature.ca.gov/faces/billNavClient.xhtml?bill_id=202520260SB782" target="_blank" rel="noopener noreferrer" style={{color:C.drd,fontWeight:600,textDecoration:"underline",textUnderlineOffset:2}}>SB 782 (2025) ↗</a></div>
                <div style={{ marginTop: 10, fontSize: 12.5, color: T.body, lineHeight: 1.6 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 16 }}>
              <div style={{ background: `${C.crd}08`, border: `1px solid ${C.crd}25`, borderRadius: 8, padding: "14px 16px" }}>
                ...
                {["No disaster required — can form anytime", "Any special district can participate (not just cities/counties)", "Standard democratic protections (protest rights intact)", "Can address any climate risk, not tied to specific event"].map((pt, i) => (
                  <div key={i} style={{ display: "flex", gap: 8, marginBottom: 5 }}>
          ...
          <section ref={setRef("why")} data-section="why" style={{ marginBottom: 70 }}>
            ...
            <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 10, padding: 20 }}>
              ...
              <p style={{ fontSize: 13.5, color: "#7f1d1d", lineHeight: 1.8, margin: 0 }}>
                <strong>Schools always lose.</strong> None of these districts can capture the school portion of property taxes — roughly half of all property tax revenue in California. This dramatically limits total increment compared to the RDA era, when the state backfilled school shortfalls. CEQA review also still applies to individual projects even under expedited DRD formation.
              </p>
            </div>
          </section>
            ...
            {[
              {
                ...
                links: [
                  ...
                  { label: "SB 782 (2025) — Disaster Recovery District", sub: "Pérez · Urgency statute, signed October 10, 2025", url: "https://leginfo.legislature.ca.gov/faces/billNavClient.xhtml?bill_id=202520260SB782" },
                ],
              },
              {
